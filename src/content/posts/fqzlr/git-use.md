...
转载自：https://fqzlr.com/posts/blog/git-use/
...
```bash
# 开了代理但 git clone 还是慢，但下载 ZIP 包很快——这是因为 Git 默认不会自动使用系统代理，需要手动配置。
# Git（特指命令行操作）和系统的网络是分开管理的，浏览器能走代理，不代表 Git 也会跟着走。
...
## 账号配置
# 全局用户名邮箱
git config --global user.name "plx00"
git config --global user.email "shook123@qq.com"
# 单仓库覆盖配置
git config user.name "plx00"
git config user.email "shook123@qq.com"
# 校验配置
